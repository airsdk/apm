...
package com.apm.client.config.processes
{
	...
	public class LoadLinuxJavaHomeProcess extends ProcessBase
	{
		////////////////////////////////////////////////////////
		//  CONSTANTS
		//
		...
		public function LoadLinuxJavaHomeProcess( config:RunConfig )
		{
		...
		override public function start( completeCallback:Function = null, failureCallback:Function = null ):void
		{
			...
			if (!isJavaHomeValid())
			{
				if (NativeProcess.isSupported)
				{
					...
					processArgs.push( "-c" );
					processArgs.push( "readlink -f $(which java)" );

					var processStartupInfo:NativeProcessStartupInfo = new NativeProcessStartupInfo();
					processStartupInfo.executable = new File( "/usr/bin/bash" );
		...
		private function onExit( event:NativeProcessExitEvent ):void
		{
			...
			if (event.exitCode == 0)
			{
				_config.env[ "JAVA_HOME" ] = _data.substr(0, Math.max( 0, _data.length - "/bin/java".length));
			}
			complete();
		}
